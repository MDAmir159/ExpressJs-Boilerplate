...
async function createUserQuery(inputArray) {
  const sqlQuery = "INSERT INTO user (id, userName, normalizedUserName, email, normalizedEmail, passwordHash, isActive, creationTime) VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP())";
  return createUpdateDelete(sqlQuery, inputArray);
}

async function getUserByEmailQuery(email) {
  const sqlQuery = `SELECT * from user WHERE normalizedEmail="${email}" AND isDeleted=0`;
  return readData(sqlQuery);
}

async function getUserRoleNameByUserIdQuery(inputObj) {
  const { userId } = inputObj;
  const sqlQuery = `SELECT r.name as roleName FROM userroles AS actual LEFT JOIN user AS u ON u.id = actual.userId LEFT JOIN roles as r ON r.id = actual.roleId WHERE u.id='${userId}';`
  return readData(sqlQuery);
}
